...
_note_title_cache: Dict[str, str] = {}
...
async def update_xhs_note(note_item: Dict):
    ...
    video_url = ','.join(get_video_url_arr(note_item)) or note_item.get('video_url', '') or ''
    ...
    local_db_item = {
        ...
        "source_keyword": note_item.get("source_keyword") or source_keyword_var.get(),  # Search keyword (creator_keyword 模式下由过滤器写入)
        "xsec_token": note_item.get("xsec_token"),  # xsec_token
    }
    utils.logger.info(f"[store.xhs.update_xhs_note] xhs note: {local_db_item}")
    await XhsStoreFactory.create_store().store_content(local_db_item)

    title = local_db_item.get("title") or local_db_item.get("desc", "")
    if note_id and title:
        _note_title_cache[str(note_id)] = str(title)
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    note_title = _note_title_cache.get(str(note_id), "")
    local_db_item = {
